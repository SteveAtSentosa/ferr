...
- `src/`: TypeScript source (`index.ts`, `ferr.ts`, `ferrAccess.ts`, `utils.ts`).
- `dist/`: compiled CJS/ESM output (generated by `pnpm build`; do not edit directly).
- `tests/`: Vitest tests (`ferr.test.ts`) and fixtures (`testData.ts`).
...
- Style: ESLint + TypeScript ESLint rules (see `.eslintrc.json`).
- Files: lowerCamelCase for module filenames (e.g., `ferrAccess.ts`).
- Exports: functional/utility naming (e.g., `makeFerrWithDefaults`, `fErrToMessageStr`).
...
- Commit messages are short, imperative, and often reference versions or a brief change summary (e.g., `add updateErrInfo()`, `2.1.14`).
- PRs should include:
